            ...
            GnomeStickyNotesRichTextToggle  which)
{
	switch (which)
		{
		case GNOME_STICKY_NOTES_RICH_TEXT_BOLD:          return self->tag_bold;
		case GNOME_STICKY_NOTES_RICH_TEXT_ITALIC:        return self->tag_italic;
		case GNOME_STICKY_NOTES_RICH_TEXT_UNDERLINE:     return self->tag_underline;
		case GNOME_STICKY_NOTES_RICH_TEXT_STRIKETHROUGH: return self->tag_strike;
		default:                                         return NULL;
		}
}
...
{
	if (g_strcmp0 (s, "center") == 0) return GTK_JUSTIFY_CENTER;
	if (g_strcmp0 (s, "right") == 0)  return GTK_JUSTIFY_RIGHT;
	if (g_strcmp0 (s, "fill") == 0)   return GTK_JUSTIFY_FILL;
	...
	for (GSList *l = tags; l != NULL; l = l->next)
		{
			...
			if (tag == self->tag_bold)            { out->bold = TRUE;      continue; }
			if (tag == self->tag_italic)          { out->italic = TRUE;    continue; }
			if (tag == self->tag_underline)       { out->underline = TRUE; continue; }
			if (tag == self->tag_strike)          { out->strike = TRUE;    continue; }
	...
	else
		{
			...
			switch (which)
				...
				case GNOME_STICKY_NOTES_RICH_TEXT_BOLD:
					self->active.bold = !self->active.bold; break;
				case GNOME_STICKY_NOTES_RICH_TEXT_ITALIC:
					self->active.italic = !self->active.italic; break;
				case GNOME_STICKY_NOTES_RICH_TEXT_UNDERLINE:
					self->active.underline = !self->active.underline; break;
				case GNOME_STICKY_NOTES_RICH_TEXT_STRIKETHROUGH:
					self->active.strike = !self->active.strike; break;
				default: break;
				}
			arm_pending (self);
			g_signal_emit (self, signals[STATE_CHANGED], 0);
                                         ...
                                         GnomeStickyNotesRichTextToggle  which)
{
	...
	switch (which)
		{
		case GNOME_STICKY_NOTES_RICH_TEXT_BOLD:          return self->active.bold;
		case GNOME_STICKY_NOTES_RICH_TEXT_ITALIC:        return self->active.italic;
		case GNOME_STICKY_NOTES_RICH_TEXT_UNDERLINE:     return self->active.underline;
		case GNOME_STICKY_NOTES_RICH_TEXT_STRIKETHROUGH: return self->active.strike;
                 ...
                 GVariantBuilder          *builder)
{
	...
	if (a.bold)      { g_variant_builder_add (builder, "{sv}", "b", g_variant_new_boolean (TRUE)); any = TRUE; }
	if (a.italic)    { g_variant_builder_add (builder, "{sv}", "i", g_variant_new_boolean (TRUE)); any = TRUE; }
	if (a.underline) { g_variant_builder_add (builder, "{sv}", "u", g_variant_new_boolean (TRUE)); any = TRUE; }
	if (a.strike)    { g_variant_builder_add (builder, "{sv}", "s", g_variant_new_boolean (TRUE)); any = TRUE; }
	if (a.family)    { g_variant_builder_add (builder, "{sv}", "fam", g_variant_new_string (a.family)); any = TRUE; }
	if (a.has_size)  { g_variant_builder_add (builder, "{sv}", "sz", g_variant_new_double (a.size)); any = TRUE; }
	if (a.has_color)
		{
